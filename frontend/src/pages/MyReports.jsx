...
export function MyReports() {
  ...
  useEffect(() => {
    reportsApi.myList({ page, page_size: 10 })
        ...
        <table className="data-table">
          ...
          <tbody>
            {data.results.length === 0 ? (
              <tr><td colSpan={7}>No reports yet. <Link to="/reports/new">Create one</Link>.</td></tr>
            ) : (
              data.results.map((r) => (
                <tr key={r.id}>
                  <td>{r.date}</td>
                  ...
                  <td>
                    <Link to={`/reports/${r.id}/edit`} className="btn btn-sm">Edit</Link>
                  </td>
                </tr>
              ))
            )}
